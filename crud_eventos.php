...
if ($_POST) {
    ...
    if ($accion == 'crear') {
        $stmt = $mysqli->prepare("INSERT INTO eventos (propiedad_id, nombre_evento, start_date, end_date, status)
                                  VALUES (?, ?, ?, ?, ?)");
        ...
        $stmt = $mysqli->prepare("UPDATE eventos
            SET propiedad_id=?, event_name=?, start_date=?, end_date=?, status=?
            WHERE event_id=?");
...
if ($accion == 'eliminar' && $evento_id > 0) {
    $stmt = $mysqli->prepare("DELETE FROM eventos WHERE event_id=?");
...
if ($accion == 'editar' && $evento_id > 0) {
    $stmt = $mysqli->prepare("SELECT * FROM eventos WHERE event_id=?");
    $stmt->bind_param("i", $evento_id);
    $stmt->execute();
    $res = $stmt->get_result();
    ...
    <form method="post" action="?section=eventos&accion=editar&id=<?php echo $evento_id; ?>">
        <label>Propiedad:
            <select name="propiedad_id">
                <?php while($p = $propiedades->fetch_assoc()): ?>
                    <option value="<?php echo $p['propiedad_id']; ?>"
                        <?php if($p['propiedad_id'] == $evento['property_id']) echo 'selected'; ?>>
                        <?php echo htmlspecialchars($p['nombre_propiedad']); ?>
                    </option>
                <?php endwhile; ?>
            </select>
        ...
        <label>Nombre Evento:
            <input type="text" name="nombre_evento" value="<?php echo htmlspecialchars($evento['event_name']); ?>">
        </label><br><br>
        <label>Fecha Inicio:
            <input type="datetime-local" name="fecha_inicio" value="<?php echo str_replace(' ', 'T', $evento['start_date']); ?>">
        </label><br><br>
        <label>Fecha Fin:
            <input type="datetime-local" name="fecha_fin" value="<?php echo str_replace(' ', 'T', $evento['end_date']); ?>">
        </label><br><br>
        <label>Estado:
            <input type="text" name="estado" value="<?php echo htmlspecialchars($evento['status']); ?>">
        </label><br><br>
...
// LISTAR
$result = $mysqli->query("SELECT e.*, p.nombre_propiedad
                          FROM eventos e
                          JOIN propiedades p ON e.property_id = p.propiedad_id
...
<table>
    ...
    <?php while($fila = $result->fetch_assoc()): ?>
        <tr>
            <td><?php echo $fila['event_id']; ?></td>
            <td><?php echo htmlspecialchars($fila['nombre_propiedad']); ?></td>
            <td><?php echo htmlspecialchars($fila['event_name']); ?></td>
            <td><?php echo htmlspecialchars($fila['start_date']); ?></td>
            <td><?php echo htmlspecialchars($fila['end_date']); ?></td>
            <td><?php echo htmlspecialchars($fila['status']); ?></td>
            <td>
                <a href="?section=eventos&accion=editar&id=<?php echo $fila['event_id']; ?>" class="actualizar">✏</a>
                <a href="?section=eventos&accion=eliminar&id=<?php echo $fila['event_id']; ?>" class="eliminar"
